...
for a in range(numberOfIterations):
    ...
    m = int(random.uniform(0, numberOfMatches))
    while i == j or i == k or i == m or j == k or j == m or k == m:
        i = int(random.uniform(0, numberOfMatches))
        j = int(random.uniform(0, numberOfMatches))
        k = int(random.uniform(0, numberOfMatches))
        m = int(random.uniform(0, numberOfMatches))
    ...
    pointSet_Source_Perspective = np.float32([[round(locations[i][0][0]), round(locations[i][0][1])],
                                              [round(locations[j][0][0]), round(locations[j][0][1])],
                                              [round(locations[k][0][0]), round(locations[k][0][1])],
                                              [round(locations[m][0][0]), round(locations[m][0][1])]])

    pointSet_Target_Perspective = np.float32([[round(locations[i][1][0]), round(locations[i][1][1])],
                                              [round(locations[j][1][0]), round(locations[j][1][1])],
                                              [round(locations[k][1][0]), round(locations[k][1][1])],
                                              [round(locations[m][1][0]), round(locations[m][1][1])]])
    ...
    for loc in locations:
        ...
        temp[0][0] = loc[0][0]
        temp[1][0] = loc[0][1]
        HomogenicTemp = np.dot(perspectiveTransformation, temp)
        ...
        if EuclideanDistance < 10:  # Threshold for Inliers
            countInliers += 1

    if countInliers > maxInliers:
        maxInliers = countInliers
        MaxInliersMatchesIndexes = (i, j, k, m)
        homographyIndexes = (i,j,k,m)
        Homography = perspectiveTransformation
...
######################### fitting ############################
x = homographyIndexes[0]
y = homographyIndexes[1]
z = homographyIndexes[2]
w = homographyIndexes[3]
pointSet_Source_Perspective = np.float32([[round(locations[x][0][0]), round(locations[x][0][1])],
                                          [round(locations[y][0][0]), round(locations[y][0][1])],
                                          [round(locations[z][0][0]), round(locations[z][0][1])],
                                          [round(locations[w][0][0]), round(locations[w][0][1])]])

pointSet_Target_Perspective = np.float32([[round(locations[x][1][0]), round(locations[x][1][1])],
                                          [round(locations[y][1][0]), round(locations[y][1][1])],
                                          [round(locations[z][1][0]), round(locations[z][1][1])],
                                          [round(locations[w][1][0]), round(locations[w][1][1])]])

perspectiveTransformation = cv2.getPerspectiveTransform(pointSet_Source_Perspective,
                                                        pointSet_Target_Perspective)
shap = np.shape(img2)
rows = shap[0]
cols = shap[1]
imgOutput = cv2.warpPerspective(img1, perspectiveTransformation, (round(cols * 1.5), round(rows*1.5)))
...
count += 1
cv2.imshow("Perspective RANSAC Pair 1", imgOutput)
#cv2.imwrite("Perspective RANSAC Pair 1.jpg", imgOutput)
cv2.waitKey(0)
